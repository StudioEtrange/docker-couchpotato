...
GITHUB_REPO="RuudBurger/CouchPotatoServer"
...
version_number=$(echo $version_name | sed -e 's/build\///g')
echo " * Process last release $version_name"
echo " ** version number : $version_number"

sed -i .bak -e "s,ENV COUCHPOTATO_VERSION.*,ENV COUCHPOTATO_VERSION $version_name," "Dockerfile"
rm -f Dockerfile.bak
...
for rel in $releases; do
	version_name="$rel"
	version_number=$(echo $version_name | sed -e 's/build\///g')
	echo " * Process release name : $version_name"
	echo " ** version number : $version_number"

	mkdir -p "$version_number"
	cp -f supervisord* "$version_number"
	cp -f Dockerfile "$version_number/Dockerfile"
	sed -i .bak -e "s,ENV COUCHPOTATO_VERSION.*,ENV COUCHPOTATO_VERSION $version_name," "$version_number/Dockerfile"
	rm -f "$version_number/Dockerfile.bak"
	echo
done

echo "******** UPDATE README ********"
list_release=$(echo $releases | sed -e 's/build\///g' | sed -e 's/ /\, /g')
last_release_tag=$(echo $last_release | sed -e 's/build\///g')
